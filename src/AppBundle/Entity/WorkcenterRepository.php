...
class WorkcenterRepository extends EntityRepository
{
    public function getVisitRelatedWorkcenters(User $user)
    {
        ...
        return $em->createQuery('SELECT DISTINCT w,
              ...
              (SELECT MAX(v4.when) FROM AppBundle:Visit v4 WHERE v4.workcenter = w AND v4.tutor = :user),
              (SELECT MIN(v5.when) FROM AppBundle:Visit v5 WHERE v5.workcenter = w AND v5.tutor = :user)
