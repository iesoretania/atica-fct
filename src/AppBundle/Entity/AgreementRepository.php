...
class AgreementRepository extends EntityRepository
{
    ...
    public function getActivitiesStats(Agreement $agreement)
    {
        ...
        $result = $this->getEntityManager()
            ->createQuery('SELECT a, SUM(t.hours), COUNT(a) FROM AppBundle:Activity a LEFT JOIN AppBundle:Tracking t WITH t.activity = a.id LEFT JOIN AppBundle:Workday w WITH t.workday = w WHERE w.agreement = :agreement AND a.id IN (:activities) GROUP BY a.code, a.id')
            ->setParameter('agreement', $agreement)
